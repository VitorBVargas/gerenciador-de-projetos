...
import { getEducationTasksForProduct, productHasEducationTasks } from '../components/homologation/homologationTasksEducation';
...
export default function Homologation() {
  ...
  React.useEffect(() => {
    if (selectedProduct && products.length > 0 && tasks.length >= 0) {
      ...
      if (product) {
        const existingTasks = tasks.filter(t => t.product_id === product.id);
        if (existingTasks.length === 0 && (productHasHomologation(product.name) || productHasEducationTasks(product.name))) {
  ...
  React.useEffect(() => {
    if (allEntities.length > 0 && !selectedEntity) {
      ...
      const entityWithProducts = allEntities.find(entity => {
        const entityProds = products.filter(p => p.entity === entity && (productHasHomologation(p.name) || productHasEducationTasks(p.name)));
  ...
  const productsByVertical = entityFilteredProducts.reduce((acc, product) => {
    if (productHasHomologation(product.name) || productHasEducationTasks(product.name)) {
  ...
  const productsWithHomologation = entityFilteredProducts.filter(p => productHasHomologation(p.name) || productHasEducationTasks(p.name));
