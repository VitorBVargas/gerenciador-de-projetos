...
export default function Homologation() {
  ...
  const createDefaultTasks = async (product) => {
    ...
    const creationKey = `${product.id}::${entityKey}`;
    if (creatingTasksRef.current.has(creationKey)) return;
    
    const existingTasks = tasks.filter(t => t.product_id === product.id && (t.entity || '') === entityKey);
    if (existingTasks.length > 0) return;

    const defaultSections = getDefaultTasksForProduct(product.name);
    if (!defaultSections) return;

    creatingTasksRef.current.add(creationKey);
    ...
    creatingTasksRef.current.delete(creationKey);
  };
    ...
    for (const row of rawData) {
      ...
      if (colA === 'etapa') {
        ...
        if (!seenTitles.has(title.toLowerCase())) {
          ...
          tasksToCreate.push({ title, project_id: projectId, entity: product.entity || '', product_id: product.id, completed: false, order: order++ });
        }
      }
    }
